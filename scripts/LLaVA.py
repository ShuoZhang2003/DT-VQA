...
def _get_args():
    ...
    parser.add_argument("--model_path", type=str, default="./model_weights/LLaVA-7B")
    parser.add_argument("--save_name", type=str, default="llava7b")
...
def eval_worker(args, data, eval_id, output_queue):
    ...
    for i in tqdm(range(len(data))):
        ...
        input_ids = tokenizer_image_token(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(f"cuda:{eval_id}")
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                input_ids,
                images=image_tensor.unsqueeze(0).half().to(f"cuda:{eval_id}"),
